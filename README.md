...
POST /run-simulation: Starts a new simulation.
Body: { "tlb_entries": int, "num_frames": int, "rep_policy": str, "addresses": str|null, "test_file": str|null }
POST /gerar-trace: Generates a new trace file.
Body: { "algoritmo": str, "nome_arquivo": str, "num_enderecos": int, ... }
GET /list-tests: Returns a JSON list of available .in test files (e.g., {"tests": ["trace1.in", "trace2.in"]}).
GET /current-stats: Provides a real-time poll of the currently running simulation's stats.

Response: { "tlb_hits": int, "tlb_misses": int, "page_faults": int }
****
