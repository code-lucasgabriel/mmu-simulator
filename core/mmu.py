...
from modules.rep_policy import BaseRepPolicy, LRU
...
class MMU:
    ...
    def search_tlb(self, page_number: int) -> bool:
    ...
    def store_page_tlb(self, num_tlb_entries: int, page_number: int, frame_number: int) -> None:
        """
        Updates the tlb with (page_number, frame_number) using the default TLU replacement policy.
        """
        self.tlb_rep_policy.update_table(self.tlb, num_tlb_entries, page_number, frame_number)

    def store_page_frame(self, rep_policy: BaseRepPolicy, num_frames: int, page_number: int) -> int:
        """
        stores a new frame into memory, using rep_policy if needed to substitute an existing frame.
        """
        frame_number = rep_policy.update_table(self.frames, self.page_table, page_number)
        return frame_number

    def _alocate_frames(self): # TODO
        """
        Internal function to alocate the frames in memory
        """
        pass